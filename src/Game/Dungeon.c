...
void LoadRoom4C(Player* player,Dungeon*dj){
    switch (dj->direction)
    {
        case 'N':
            ...
            break;
        case 'S':
            glm_vec3_copy((vec3){-2.2f,0.0f,-3.5f}, player->playerModel->position);
            break;
        case 'W':
            glm_vec3_copy((vec3){3.8f,0.0f,-2.8f}, player->playerModel->position);
            player->playerModel->rotation[1] =glm_rad(-90.0f);
...
void LoadRoom2I(Model* map,Model* col, Player* player,Dungeon * dj) {
    ...
    case 'E':
        map->rotation[1] = glm_rad(90.0f);
        col->rotation[1] = glm_rad(90.0f);
        glm_vec3_copy((vec3){-4.0f,0.0f,0.0f}, player->playerModel->position);
        player->playerModel->rotation[1] =glm_rad(90.0f);
        break;
    case 'W':
        map->rotation[1] = glm_rad(90.0f);
...
void LoadRoom3T(Model* map,Model* col, Player* player,Dungeon *dj) {
    ...
    {
    case 'S':
        glm_vec3_copy((vec3){-0.0f,0.0f,-6.5f}, player->playerModel->position);
        break;
    case 'N':
        glm_vec3_copy((vec3){0.0f,0.0f,6.5f}, player->playerModel->position);
        player->playerModel->rotation[1] =glm_rad(180.0f);
        break;
    case 'E':
        glm_vec3_copy((vec3){-6.5f,0.0f,0.0f}, player->playerModel->position);
        player->playerModel->rotation[1] =glm_rad(90.0f);
        break;
    case 'W':
        glm_vec3_copy((vec3){6.5f,0.0f,-0.4f}, player->playerModel->position);
        player->playerModel->rotation[1] =glm_rad(-90.0f);
        break;
    }
}
...
void LoadRoom2L(Model* map,Model* col, Player* player,Dungeon *dj) {
    ...
    case 'S':
        if (dir_used[3]){
        glm_vec3_copy((vec3){-0.5f,0.0f,-4.25f}, player->playerModel->position);
        }
        else {glm_vec3_copy((vec3){-7.3f,0.0f,-11.5f}, player->playerModel->position);}
        break;
    case 'N':
        if (dir_used[3]){
        glm_vec3_copy((vec3){7.5f,0.0f,11.0f}, player->playerModel->position);
        }
        else {glm_vec3_copy((vec3){0.15f,0.0f,3.6f}, player->playerModel->position);}
        player->playerModel->rotation[1] =glm_rad(180.0f);
        break;
    case 'E':
        if (dir_used[1]){
        glm_vec3_copy((vec3){-11.1f,0.0f,7.25f}, player->playerModel->position);
        }
        else {glm_vec3_copy((vec3){-3.8f,0.0f,-0.0f}, player->playerModel->position);}
        player->playerModel->rotation[1] =glm_rad(90.0f);
        break;
    case 'W':
        if (dir_used[1]){
        glm_vec3_copy((vec3){3.8f,0.0f,-0.11f}, player->playerModel->position);
        }
        else {glm_vec3_copy((vec3){11.2f,0.0f,-7.8f}, player->playerModel->position);}
        player->playerModel->rotation[1] =glm_rad(-90.0f);
        break;
    }
}
...
void LogicRoom2C(Dungeon*dj,Player* player ){
    ...
    glm_vec3_sub(player->playerModel->position, Door2Position, Door2Dir);
...
void LogicRoom3C(Dungeon*dj,Player* player ){
    ...
    glm_vec3_sub(player->playerModel->position, Door1Position, Door1Dir);
    glm_vec3_sub(player->playerModel->position, Door2Position, Door2Dir);
    glm_vec3_sub(player->playerModel->position, Door3Position, Door3Dir);
    glm_vec3_sub(player->playerModel->position, Door4Position, Door4Dir);
    float Door1Dist = glm_vec3_norm(Door1Dir);
    float Door2Dist = glm_vec3_norm(Door2Dir);
...
void LogicRoomB(Dungeon*dj,Player* player ){
    if (getKeyState(SDLK_n)){
    printf("On vient du %c, le player est a %f\n",dj->direction,player->playerModel->rotation[1]);} 
    vec3 DoorPosition={0.0f,0.0f,0.0f};
    vec3 DoorDir;
    char d1=dj->direction;
    switch (dj->direction){
